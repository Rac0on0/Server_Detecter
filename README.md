...
1. **관리자 권한 필요 (Admin Privileges Required)**  
   ...
   - 실행 파일(`.exe`) 우클릭 ➔ **속성** ➔ **호환성** ➔ **[관리자 권한으로 이 프로그램 실행]** 체크 후 실행하는 것을 권장합니다.  
   - Due to Raw Socket packet capturing, **administrator privileges** are required.  
   - Right-click the `.exe` ➔ **Properties** ➔ **Compatibility** ➔ Check **[Run this program as an administrator]**.

2. **전용 폴더 생성 권장 (Dedicated Folder Recommended)**  
...
소스 코드(`Server_Detecter_MK4.py`)를 직접 실행하는 경우 아래 라이브러리가 필요합니다:  
If running the Python source code (`Server_Detecter_MK4.py`) directly, the following packages are required:
